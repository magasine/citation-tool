javascript: (() => {
  ...
  const CONFIG = {
    ...
    APP_INFO: {
      name: "Citation Tool",
      version: "v20250514-shadow-tt", // Versão atualizada para Trusted Types
      credits: "by @magasine",
    },
    FORMATS: [
  ...
  // Função de sanitização (mantida como está, pois seu uso é para textContent)
  const sanitize = (str) => {
    if (!str) return "";
    return String(str)
      .replace(/&/g, "&amp;")
  ...
  const Utils = {
    ...
    showFeedback: (shadowRoot, message, duration = 3000) => {
      ...
      feedback.textContent = sanitize(message); // Usar textContent é seguro aqui
  ...
  const Citation = {
    format: (format, text, title, url, service, includeLink) => {
      ...
      const formats = {
        ...
        markdown: () =>
          `# ${safeTitle}\n\n> ${safeText.replace(/\n/g, "\n> ")}\n\n[Source](${safeUrl})` +
          (includeLink ? `\n\n[Readable](${safeServiceUrl})` : ""),
        html: () => // Esta string HTML não é injetada via innerHTML no script, apenas retornada
          `<blockquote><h2>${safeTitle}</h2><p>${safeText.replace(/\n/g, "<br>")}</p>` +
          `<footer><a href="${safeUrl}">Source</a>` +
          (includeLink ? ` | <a href="${safeServiceUrl}">Readable</a></footer></blockquote>` : "</footer></blockquote>"),
        twitter: () =>
    ...
    share: {
      whatsapp: (text) => window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, "_blank"),
      twitter: (text) => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}`, "_blank"),
      email: (text, title) => window.open(`mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(text)}`, "_blank"),
      qrCode: (text) => window.open(`https://quickchart.io/qr?text=${encodeURIComponent(text)}&size=300`, "_blank"),
    },
  };
  ...
  const UI = {
    createStyles: (shadowRoot) => {
      ...
      const cssText = `
       ...
       `;
      // Alteração para compatibilidade com Trusted Types: usar createTextNode
    ...
    createUI: (shadowRoot, hostElement) => {
      ...
      const titleElement = document.createElement("h3"); // Renomeado para evitar conflito com document.title
      ...
      Object.assign(versionBadge.style, { color: "#bcbcbc", fontSize: "0.7em", fontWeight: "300", padding: "5px" });
      ...
      Object.assign(minimizeBtn.style, { background: "none", border: "none", color: "white", cursor: "pointer", fontSize: "1.2em", marginRight: "8px", padding: "0 5px", width: "20px", textAlign: "center", lineHeight: "1" });

      const closeBtn = document.createElement("button");
      closeBtn.textContent = "×";
      Object.assign(closeBtn.style, { background: "none", border: "none", color: "white", cursor: "pointer", fontSize: "1.2em", padding: "0 5px", width: "20px", textAlign: "center", lineHeight: "1" });
      ...
      Object.assign(selectionBtn.style, { flex: "1", padding: "6px", background: state.captureMode === "selection" ? "#296fa7" : "", color: state.captureMode === "selection" ? "white" : "" });

      const clipboardBtn = document.createElement("button");
      clipboardBtn.textContent = "Clipboard";
      clipboardBtn.dataset.mode = "clipboard";
      Object.assign(clipboardBtn.style, { flex: "1", padding: "6px", background: state.captureMode === "clipboard" ? "#296fa7" : "", color: state.captureMode === "clipboard" ? "white" : "" });
      ...
      preview.textContent = selectedText; // Usa textContent, que é seguro

      const clipboardControls = document.createElement("div");
      clipboardControls.style.display = state.captureMode === "clipboard" ? "block" : "none";
      ...
      // Usa createTextNode para o texto do label, mais seguro que concatenar strings em alguns contextos
      readabilityCheckLabel.appendChild(document.createTextNode(" Include readability link"));
      ...
      Object.assign(footer.style, { padding: "8px", textAlign: "center", fontSize: "12px" });
      ...
      const updatePreview = () => {
        ...
        if (state.captureMode === "clipboard") {
          preview.textContent = state.clipboardItems.length
            ? state.clipboardItems.map((item, i) => `[${i + 1}] ${item.length > 100 ? item.substring(0, 100) + "..." : item}\n${CONFIG.CLIPBOARD_SEPARATOR}`).join("")
      ...
      [selectionBtn, clipboardBtn].forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          selectionBtn.style.background = state.captureMode === "selection" ? "#296fa7" : "";
          selectionBtn.style.color = state.captureMode === "selection" ? "white" : "";
          clipboardBtn.style.background = state.captureMode === "clipboard" ? "#296fa7" : "";
          clipboardBtn.style.color = state.captureMode === "clipboard" ? "white" : "";
          updatePreview();
        });
      });
      ...
      const getFormattedText = () => {
        ...
        const text = state.captureMode === "selection" ? preview.textContent : state.clipboardItems.join(CONFIG.CLIPBOARD_SEPARATOR);
        return Citation.format(format, text, pageTitle, pageUrl, service, includeLink);
      };
      ...
      whatsappBtn.addEventListener("click", () => Citation.share.whatsapp(getFormattedText()));
      twitterBtn.addEventListener("click", () => Citation.share.twitter(getFormattedText()));
      emailBtn.addEventListener("click", () => Citation.share.email(getFormattedText(), pageTitle));
      readabilityBtn.addEventListener("click", () => {
        const service = CONFIG.READABILITY_SERVICES[readabilitySelect.value];
        window.open(service.url(pageUrl), "_blank");
      });
      qrBtn.addEventListener("click", () => Citation.share.qrCode(getFormattedText()));

      minimizeBtn.addEventListener("click", () => {
        state.isMinimized = !state.isMinimized;
        badge.classList.toggle("minimized", state.isMinimized);
        minimizeBtn.textContent = state.isMinimized ? "+" : "−";
        Utils.showFeedback(shadowRoot, state.isMinimized ? "UI minimized" : "UI restored");
      });
  ...
  const dragElement = (element, dragHandle) => {
    ...
    const elementDrag = (e) => {
      ...
      if (newTop + hostRect.height > window.innerHeight) newTop = window.innerHeight - hostRect.height;
      if (newLeft + hostRect.width > window.innerWidth) newLeft = window.innerWidth - hostRect.width;
